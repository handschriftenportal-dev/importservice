...
import staatsbibliothek.berlin.hsp.importservice.domain.service.DigitalisateImportService;
...
public class DigitalisatImportRestControllerTest {
  ...
  @MockBean
  private DigitalisateImportService digitalisateImportService;

  @Test
  void testUpload() throws Exception {
    ImportJob job = new ImportJob(null, "test.xml", "Konrad Eichstädt");
    Mockito.when(digitalisateImportService.importDateien(any(), any(), any(), any())).thenReturn(job);

    importJobRepository.save(job);

    MockMultipartFile multipartFile = new MockMultipartFile("datei", "test.xml",
        "application/xml", "Spring Framework".getBytes());
    this.mockMvc.perform(multipart("/rest/digitalisat/import").file(multipartFile))
